...
export default function App() {
  ...
  const [data,        setData]        = useState(() => { try { return JSON.parse(localStorage.getItem("dailyplanner-v2")||"null")||INIT; } catch { return INIT; } });
  ...
  useEffect(() => { try { localStorage.setItem("dailyplanner-v2",JSON.stringify(data)); } catch {} }, [data]);
  ...
  const Chip = ({label, active, color, onClick}) => (
    <button onClick={onClick} style={{
      ...
      boxShadow: active ? `0 1px 4px ${color}22` : "none",
    }}>{label}</button>
  );
  ...
  const CatBadge = ({catKey}) => {
    ...
    return (
      <span style={{display:"inline-flex",alignItems:"center",gap:3,padding:"2px 8px",borderRadius:10,
        ...
        fontSize:9, color:c.color, fontFamily:F, fontWeight:700, whiteSpace:"nowrap"}}>
        {c.emoji} {c.label}
      </span>
    );
  };
    ...
    <div style={{display:"flex",flexWrap:"wrap",gap:5}}>
      {Object.entries(CATEGORIES).map(([k,c])=>(
        <button key={k} onClick={()=>onChange(k)} style={{
          ...
          boxShadow: value===k ? `0 1px 4px ${c.color}22` : "none",
        }}>
  ...
  const TR = ({task, onToggle, onDelete, onEdit, onSave, ekey, isSpan=false}) => {
    ...
    const rowBg     = task.done ? "#F5F0E8" : isSpan ? sc.bg : task.carried ? "rgba(192,68,10,0.05)" : "white";
    const rowBorder = task.done ? BORDER : isSpan ? sc.border : task.carried&&!task.done ? "rgba(192,68,10,0.25)" : BORDER;
    return (
      <div style={{display:"flex",alignItems:"flex-start",gap:10,padding:"10px 13px",borderRadius:10,marginBottom:6,
        background:rowBg, border:`1.5px solid ${rowBorder}`,
        boxShadow: task.done?"none":"0 1px 4px rgba(0,0,0,0.05)", transition:"all 0.15s"}}>
  ...
  const SpanPanel = ({scopeKey, span}) => {
    ...
    return (
      <div style={{marginBottom:16,padding:"13px 16px",borderRadius:12,background:sc.bg,border:`1.5px solid ${sc.border}`,boxShadow:"0 1px 4px rgba(0,0,0,0.04)"}}>
    ...
    return (
      <div style={{position:"fixed",inset:0,background:"rgba(44,26,14,0.45)",backdropFilter:"blur(6px)",zIndex:1000,display:"flex",alignItems:"center",justifyContent:"center"}}
        ...
        <div onClick={e=>e.stopPropagation()} style={{
          ...
          borderRadius:16, padding:26, width:420,
          ...
          <div style={{display:"flex",gap:8,marginBottom:16}}>
            {["week","month"].map(s=>{const c=SPAN_COLORS[s]; return (
              <button key={s} onClick={()=>setSpanType(s)} style={{
                flex:1,padding:"8px 0",borderRadius:9,cursor:"pointer",transition:"all 0.15s",
          ...
          <div style={{marginBottom:14}}>
            <div style={{fontSize:10,color:MUTED,fontWeight:700,letterSpacing:"0.04em",marginBottom:8,fontFamily:F}}>CATEGORY</div>
          ...
          <div style={{display:"flex",gap:8}}>
            ...
            <button onClick={()=>addSpanTask(sk)} style={{flex:2,padding:"9px 0",borderRadius:10,cursor:"pointer",fontFamily:F,fontSize:12,fontWeight:700,border:"none",background:sc.dot,color:"white",boxShadow:`0 2px 8px ${sc.dot}44`}}>+ Add {spanType}-span task</button>
          </div>
        </div>
      </div>
    );
  ...
  const FilterBar = () => (
    <div style={{display:"flex",flexDirection:"column",gap:10,marginBottom:16,padding:"13px 15px",borderRadius:12,background:"white",border:`1.5px solid ${BORDER}`,boxShadow:"0 1px 4px rgba(0,0,0,0.05)"}}>
  ...
  const CatFilterBar = () => (
    <div style={{display:"flex",gap:5,flexWrap:"wrap",alignItems:"center",marginBottom:16,padding:"11px 14px",borderRadius:11,background:"white",border:`1.5px solid ${BORDER}`,boxShadow:"0 1px 4px rgba(0,0,0,0.04)"}}>
      <span style={{fontSize:10,color:MUTED,fontWeight:700,letterSpacing:"0.04em",marginRight:3,fontFamily:F}}>CATEGORY</span>
      <Chip label="All" active={filterCat==="all"} color={PRIMARY} onClick={()=>setFilterCat("all")}/>
      {Object.entries(CATEGORIES).map(([k,c])=>(
        <Chip key={k} label={`${c.emoji} ${c.label}`} active={filterCat===k} color={c.color} onClick={()=>setFilterCat(k)}/>
  ...
  return (
    <div style={{minHeight:"100vh",background:BG,color:TEXT,fontFamily:F}}>
        ...
        <div style={{display:"flex",gap:5}}>
          {["day","week","month"].map(v=>(
            <button key={v} onClick={()=>setView(v)} style={{
              padding:"6px 16px",borderRadius:8,cursor:"pointer",transition:"all 0.15s",
            ...
            <div>
              <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:18}}>
                ...
                <div style={{fontFamily:F,fontSize:22,fontWeight:700,letterSpacing:"-0.4px",color:TEXT}}>
                  {fmt(selObj)}
                  {selDate===todayKey&&<span style={{marginLeft:10,fontSize:9,color:PRIMARY,background:"rgba(192,68,10,0.10)",padding:"3px 9px",borderRadius:20,verticalAlign:"middle",fontWeight:700,letterSpacing:"0.05em",border:`1px solid rgba(192,68,10,0.22)`}}>TODAY</span>}
                </div>
                <NavBtn onClick={()=>{const d=fromKey(selDate);d.setDate(d.getDate()+1);setSelDate(toKey(d));}} label="›"/>
              ...
              {dayTasks.length>0&&(
                <div style={{marginBottom:16,padding:"12px 15px",borderRadius:11,background:"white",border:`1.5px solid ${BORDER}`,boxShadow:"0 1px 4px rgba(0,0,0,0.05)"}}>
              ...
              <div style={{marginTop:18,padding:"16px",borderRadius:12,background:"white",border:`1.5px solid ${BORDER}`,boxShadow:"0 1px 6px rgba(0,0,0,0.06)"}}>
                <div style={{marginBottom:10}}>
                  <div style={{fontSize:10,color:MUTED,fontWeight:700,letterSpacing:"0.05em",marginBottom:8,fontFamily:F}}>CATEGORY FOR NEW TASK</div>
        ...
        {view==="week"&&(
          <div>
            <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:20}}>
              ...
              <div style={{fontFamily:F,fontSize:21,fontWeight:700,letterSpacing:"-0.4px",color:TEXT}}>
            ...
            {(()=>{
              ...
              return (
                <div style={{padding:"14px 18px",borderRadius:12,background:"white",border:`1.5px solid ${BORDER}`,marginBottom:18,display:"flex",gap:24,flexWrap:"wrap",boxShadow:"0 1px 4px rgba(0,0,0,0.05)"}}>
        ...
        {view==="month"&&(
          <div>
            <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:20}}>
              ...
              <div style={{fontFamily:F,fontSize:21,fontWeight:700,letterSpacing:"-0.4px",color:TEXT}}>{MONTHS_FULL[monthAnchor.getMonth()]} {monthAnchor.getFullYear()}</div>
              <NavBtn onClick={()=>{const d=new Date(monthAnchor);d.setMonth(d.getMonth()+1);setMonthAnchor(d);}} label="›"/>
      ...
      <div style={{borderTop:`1.5px solid ${BORDER}`,padding:"14px 28px",display:"flex",gap:14,flexWrap:"wrap",alignItems:"center",background:"white"}}>
        ...
        <div style={{fontSize:10,color:"#C4B0A0",marginLeft:"auto",fontFamily:F,fontWeight:500}}>Double-click to edit · Enter to save</div>
      </div>
